...
function wf_process_orders($orderid, string $apiurl, array $credentials, $storeSuffix)
{
    ...
    foreach ($order->get_items() as $item_key => $item) {
        ...
        $product_image_url = $variation_id ? $productobj->get_image_url() : $productobj->get_image_url('full');
    ...
    if (!empty($_POST)) {
            ...
            if (isset($row['select'])) {
                ...
                if (is_wp_error($response)) {
                    ...
                    if ($res->success == "success") {
                        ...
                        update_post_meta($postID, 'fmDraftSynced', 'yes');
                        update_post_meta($postID, 'fmDraftOrderNumber', $res->OrderNo);
                    } else {
        ...
        if ($loop->have_posts()):
            while ($loop->have_posts()): $loop->the_post();
                ...
                if (in_array($order_shipping_country, $configCountries)) {
